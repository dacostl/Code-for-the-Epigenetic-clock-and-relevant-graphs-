...
create_model <- function(iteration) {
  ...
  input_filename <- paste0(c("starting_subset_data_",iteration,".csv"), collapse = "")
  output_filename <- paste0(c("starting_subset_data_",(iteration+1),".csv"), collapse = "")
  ...
  read_csv(input_filename)-> starting_subset_data
  trafo= function(x,adult.age=20) { x=(x+1)/(1+adult.age); y=ifelse(x<=1, log( x),x-1);y }
  starting_subset_data%>% mutate(fage = trafo(age))->Mutated_Data_subset
  ...
  ggplot(Converted_prediction_vs_actual_training, aes(x = .pred_class, y = Age)) +
    ...
    ggtitle(paste0("Actual age vs Predicted age (Training Data) - Correlation: ", round(correlation_train$cor, 2), " / Median Error: ", round(Median_error_training, 2)))
    ggsave(filename = training_graph,device = "svg")

  
  Converted_prediction_vs_actual_testing%>%ggplot(aes(x=.pred_class, y=Age))+ geom_point(colour="#009E73")+xlab("Predicted age")+ ylab("Actual age")+ ggtitle(paste0("Actual age vs Predicted age (Testing data) - Correlation: ", round(correlation_test$cor, 2), " / Median Error: ", round(Median_error_testing, 2)))+geom_abline()
 ggsave(filename = testing_graph,device = "svg")
  ...
  starting_subset_data%>%select(!any_of(relevant_CpGs_coefficients$term))->New_starting_data
